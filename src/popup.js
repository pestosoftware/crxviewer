...
initialize();
function parseFileNameFromURL(chrome_url) {
    // We will parse web-client-for-instagram from  cws_url = https://chromewebstore.google.com/detail/web-client-for-instagram/cfegchignldpfnjpodhcklmgleaoanhi
    chrome_url = URL(url);
    var path = chrome_url.pathname;
    path = path.replace(/\/$/, '');
    path = path.split('/');
    var my_filename = path[path.length - 2];
    return my_filename;
}
...
function initialize() {
    ...
    chrome.tabs.query({
        ...
        for (var i = 0; i < tabs.length; i++) {
            ...
            var theGoodFileName = parseFileNameFromURL(cws_url);
            filename = get_zip_name(crx_url, theGoodFileName);
            if (can_viewsource_crx_url(crx_url)) {
                doDownload(crx_url, filename);
            }
...
function doDownload(crx_url, filename) {
    openCRXasZip(crx_url, function (blob, publicKey) {
        tryTriggerDownload(blob, filename);
    }, function (errorMessage) {
        console.error(errorMessage);
